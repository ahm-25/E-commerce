    ...
    <div class="login-container">
      <div class="login-card">
        <div class="login-header text-center mb-8">
          <h1 class="text-h2 font-semibold mb-2">{{ t('auth.welcomeBack') }}</h1>
          <p class="text-secondary text-sm">{{ t('auth.loginSubtitle') }}</p>
        </div>
        ...
        <form @submit.prevent="handleLogin" class="login-form">
          ...
          <div class="form-group mb-6">
            <div class="flex justify-between items-center mb-2">
              ...
              <NuxtLinkLocale to="/forgot-password" class="text-xs text-accent hover:underline">{{ t('auth.forgotPassword') }}</NuxtLinkLocale>
            </div>
        ...
        <p class="text-center text-sm text-secondary mt-8">
          {{ t('auth.noAccount') }} <NuxtLinkLocale to="/register" class="text-accent font-medium hover:underline">{{ t('auth.signUp') }}</NuxtLinkLocale>
        </p>
      </div>
    </div>
  </div>
...
const handleLogin = () => {
  ...
  setTimeout(() => {
    ...
    navigateTo(localePath('/')) // Go back to home or account page
  }, 1500)
}
</script>
...
<style scoped>
.login-page {
  min-height: calc(100vh - 5rem); /* Assuming header is 5rem */
  display: flex;
  align-items: center;
...
[data-theme="light"] .login-card, :root .login-card {
